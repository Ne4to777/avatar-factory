...
| **MuseTalk** | Real-time lip-sync анимация лица (замена SadTalker) |
...
**Симптом:** Лог показывает `SadTalker` вместо `MuseTalk`

**Решение:** Перезапустите сервер для загрузки нового кода:
```cmd
stop.bat
start.bat
```
...
| **MuseTalk (10 сек видео)** | **10–15 сек** (в 3–4 раза быстрее SadTalker!) |
| Stable Diffusion XL | 5–10 сек |

**Итого:** ~15–25 сек на одно видео (было 40–70 сек со SadTalker).

**MuseTalk** работает значительно быстрее благодаря real-time архитектуре и оптимизированному inference.
